...
public class Game {
	...
	/**
	 * Slows down the player if needed. It halfs the roll value that
	 * the player has thrown and rounds it down if the value is uneven.
	 * 
	 * @param 		roll
	 * @return		half the value of the roll, rounded down
	 */
	private int slowDownIfNeeded(int roll) {
		if(currentPlayer().slowDownNextTurn) {
			if(roll%2 == 1)
				roll++;
			return roll/2;
		}
